...
class Ccc_Process_Adminhtml_Process_TypeController extends Mage_Adminhtml_Controller_Action{
    ...
    {
        $content = $this->getLayout()->createBlock('process/adminhtml_process_type')->toHtml();
        $response = [
            'status'=>'success',
    ...
    {
        $content = $this->getLayout()->createBlock('process/adminhtml_process_type_edit')->toHtml();
        $response = [
            'status'=>'success',
            'message'=>'process grid',
            'element'=>[
        ...
        // $processTypeModel = Mage::getModel('process/process_type')->load( $processTypeId);
        ...
        //     $this->_addContent($this->getLayout()->createBlock('process/adminhtml_process_type_edit'));
        //     $this->renderLayout();

        // }
        // else{
        //     Mage::getSingleton('adminhtml/session')->addError(Mage::helper('process')->__('process type does not exist'));
        //     $this->_redirect('*/*/');
        // }
    }
    public function saveAction()
    {
        if($this->getRequest()->getPost()){
          try{
                ...
                $processTypeModel = Mage::getModel('process/process_type')->load($id);
    ...
    public function deleteAction()
    {
        if($this->getRequest()->getParam('id') > 0){
            try{
               $processTypeModel = Mage::getModel('process/process_type');
               $processTypeModel->setId($this->getRequest()->getParam('id'))
                ->delete();
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')
                ->__('process type was successfully deleted'));
                $this->_redirect('*/*/');
            }catch(Exception $e){
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $this->_redirect('*/*/edit',array('id'=>$this->getRequest()->getParam('id')));
